...
$bssubject = " $company, Booth # $booth, ACIC BoothSeal Order";
...
$ez_shelf . $misc_furn . $autopoles . $carpet . $wallpaper . $companyinfo . $mop . $totalq . $date;

$from = "From: ACIC EXPRESS FORM - GES";


mail("mevans@wsashow.com, ssantamaria@enkshows.com", $subject,  $message, $from);
mail("enkorders@ges.com", $subject, "$message \f $companyinfo $ccinfo", $from);
...
$emessage = $emessage . $companyinfo . $mop .  $etotalq . $edate;

mail("mevans@wsashow.com, ssantamaria@enkshows.com", $esubject, $emessage, $efrom);
mail("enk@portparties.com", $esubject, "$emessage $ccinfo", $efrom);
...
$bsmessage = $bsmessage . $companyinfo . $mop . $bstotalq . $bsdate;

mail("mevans@wsashow.com, ssantamaria@enkshows.com", $bssubject, $bsmessage, $bsfrom);
mail("dmendoza@bootseal.com, boruchow@cckevents.com", $bssubject, "$bsmessage $ccinfo", $bsfrom);

$bsmessage = "Thank you for your request. This is a summary of your BoothSeal request which has been submitted to BoothSeal LLC for the May 2012 CIRCUIT/INTERMEZZO. THIS IS NOT A CONFIRMATION. 
...
Orders received without full payment will not be processed until BoothSeal LLC receives payment by either check, wire transfer or credit card. If you have not done so already, go to  http://www.enkforms.com/expressform/acic/pdfs/ACIC_Guide_P59.pdf to download the BoothSeal credit card authorization form for your convenience.

Reminder:  Do not duplicate your order by either resubmitting the Express Order form or by completing similar forms from the GuideBook.  If you do so, your order will be duplicated and your will be charged twice. 

" . $bsmessage ; 

mail($email, "Circuit.Intermezzo BoothSeal", $bsmessage, $bsfrom); }
